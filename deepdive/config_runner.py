...
from .plots import (
    add_geochrono_no_labels,
    features_pca,
    features_through_time,
    plot_dd_predictions,
    plot_ensemble_predictions,
)
from .utilities import get_instance_mse, get_r_squared
...
def run_config(
    ...
    if test_set is not None:
        ...
        if "simulations" in config.sections() and config.getint(
            "simulations", "n_test_simulations"
        ):
            test_feature_file, test_label_file, test_totdiv_label_file = (
                run_test_sim_from_config(config)
            )
    ...
    if trained_model is None:
        ...
        for model_i in range(n_models):
            if feature_file is not None and "model_training" in config.sections():
                model_tmp = run_model_training_from_config(
                    config,
                    feature_file=feature_file,
                    label_file=label_file,
                    model_tag=out_tag,
                    return_model_dir=True,
                    calibrate_output=calibrated,
    ...
    if test_feature_file is not None and model_dir is not None:
        ...
        for model_i in range(len(model_dir)):
            test_pred, labels, testset_features = predict_testset_from_config(
                config,
                test_feature_file,
                test_label_file,
            ...
            print(
                "Saved testset predictions in:\n",
                os.path.join(model_dir[model_i], pred_file),
            )

            with open(
                file=os.path.join(
                    model_dir[model_i], pred_file.replace(".npy", "MSA.txt")
                ),
                mode="w",
            ) as f:
                f.write(
                    "Model %s Test set MSE: %s"
                    % (model_i, np.mean((test_pred - labels) ** 2))
                )

            try:
                if total_diversity is False:
                    instance_acccuracy_m = np.array(
                        [
                            get_r_squared(test_pred, labels),
                            get_instance_mse(test_pred, labels),
                        ]
                    ).T
                    instance_acccuracy.append(instance_acccuracy_m)
                else:
                    instance_acccuracy_m = np.array(
                        [np.ones(len(labels)), (test_pred - labels) ** 2]
                    ).T
                    instance_acccuracy.append(instance_acccuracy_m)
            except:
    ...
    if "empirical_predictions" in config.sections():
        ...
        dd_input = os.path.join(
            config["general"]["wd"],
            config["empirical_predictions"]["empirical_input_file"],
        )
        tbl = pd.read_csv(dd_input, keep_default_na=False)
        ...
        for model_i in range(len(model_dir)):
            pred_div, feat = predict_from_config(
                config,
                return_features=True,
                model_tag=out_tag,
                calibrated=calibrated,
            ...
            if testset_features is not None:
                ...
                if model_i == 0:
                    plot_feature_hists(
                        test_features=testset_features,
            ...
            feat_tbl.to_csv(
                os.path.join(model_dir[model_i], "Empirical_features_%s.csv" % out_tag),
                index=False,
            )
            ...
            predictions.to_csv(
                os.path.join(
                    model_dir[model_i], "Empirical_predictions_%s.csv" % out_tag
                ),
                index=False,
            )
        ...
        with open(
            os.path.join(
                config["general"]["wd"],
                config["model_training"]["model_folder"],
                "config_settings.txt",
            ),
            "w",
        ) as configfile:
...
def sim_and_plot_features(config_file, wd=None, CPU=None, n_sims=None):
    ...
    test_feature_file, test_label_file, totdiv_label_file = run_test_sim_from_config(
        config
    )
    testset_features = np.load(test_feature_file)
    feature_plot_dir = os.path.join(
        config["general"]["wd"],
