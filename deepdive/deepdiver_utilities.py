...
def create_sim_obj_from_config(config, rseed=None):
    ...
    if "fixed_mass_extinction" in config["simulations"]:
        try:
            fixed_mass_extinction = list(
                map(int, config["simulations"]["fixed_mass_extinction"].split())
            )
        except ValueError:
    ...
    if "survive_age_condition" in config["simulations"]:
        try:
            survive_age_condition = float(
                config["simulations"]["survive_age_condition"]
            )
        except ValueError:
    ...
    bd_sim = bd_simulator(
        ...
        min_extant_sp=np.min(
            list(map(float, config["simulations"]["extant_sp"].split()))
        ),
        max_extant_sp=np.max(
            list(map(float, config["simulations"]["extant_sp"].split()))
        ),
        ...
        magnitude_mass_ext=list(
            map(float, config["simulations"]["magnitude_mass_ext"].split())
        ),
        fixed_mass_extinction=fixed_mass_extinction,
        p_mass_speciation=float(config["simulations"]["p_mass_speciation"]),
        poiL=config.getfloat("simulations", "poil"),
        poiM=config.getfloat("simulations", "poim"),
    ...
    try:
        ...
        bin_mean_rates = np.array(
            list(map(float, config["simulations"]["bin_mean_rates"].split()))
        )
        bin_std_rates = np.array(
            list(map(float, config["simulations"]["bin_std_rates"].split()))
        )
    except:
        freq_bin_sampling = 0
        bin_std_rates = None
    ...
    try:
        ...
        singleton = list(
            map(float, config["simulations"]["singletons_frequency"].split())
        )

    if "locality_rate_multiplier" in config["simulations"]:
        locality_rate_multiplier = list(
            map(float, config["simulations"]["locality_rate_multiplier"].split())
        )
    else:
        locality_rate_multiplier = None

    if "species_per_locality_multiplier" in config["simulations"]:
        species_per_locality_multiplier = list(
            map(float, config["simulations"]["species_per_locality_multiplier"].split())
        )
    else:
        species_per_locality_multiplier = None

    time_bins = np.sort(
        np.array(list(map(float, config["general"]["time_bins"].split())))
    )

    if "locality_data_file" in config["simulations"]:
        bin_mean_rates = np.load(
            os.path.join(
                config["general"]["wd"], config["simulations"]["locality_data_file"]
            )
        )
        # print("Loaded locality data file", np.exp(bin_mean_rates))

        fossil_sim = fossil_simulator_empirical(
            n_areas=config.getint("general", "n_regions"),
            n_bins=len(list(map(float, config["general"]["time_bins"].split())))
            - 1,  # number of time bins
            time_bins=time_bins,
            sp_mean=list(map(float, config["simulations"]["sp_mean"].split())),
            sp_variance=config.getfloat("simulations", "sp_variance"),
            target_n_occs=target_n_occs,
            target_n_occs_range=target_n_occs_range,
            seed=rseed,
            empirical_localities=bin_mean_rates,
        )

    else:
        fossil_sim = fossil_simulator(
            n_areas=config.getint("general", "n_regions"),
            n_bins=len(list(map(float, config["general"]["time_bins"].split())))
            - 1,  # number of time bins
            time_bins=time_bins,
            eta=list(
                map(float, config["simulations"]["eta"].split())
            ),  # area-sp stochasticity
            p_gap=list(
                map(float, config["simulations"]["p_gap"].split())
            ),  # probability of 0 preservation in a time bin
            dispersal_rate=config["simulations"]["dispersal_rate"],
            max_dist=config.getfloat("simulations", "max_dist"),
            disp_rate_mean=list(
                map(float, config["simulations"]["disp_rate_mean"].split())
            ),
            ...
            size_concentration_parameter=list(
                map(
                    float, config["simulations"]["size_concentration_parameter"].split()
                )
            ),
            # single value or array of length n_areas
            link_area_size_carrying_capacity=list(
                map(
                    float,
                    config["simulations"]["link_region_size_carrying_capacity"].split(),
                )
            ),
            # positive, larger numbers = stronger link between area size and carrying capacity
            p_origination_a_slope_mean=config.getfloat(
                "simulations", "p_origination_a_slope_mean"
            ),
            # mean slope of probability of origination area mean
            p_origination_a_slope_sd=config.getfloat(
                "simulations", "p_origination_a_slope_sd"
            ),  # std of the slopes
            sp_mean=list(
                map(float, config["simulations"]["sp_mean"].split())
            ),  # G(a,b) distributed preservation rates across species
            sp_variance=config.getfloat("simulations", "sp_variance"),
            slope=list(
                map(float, config["simulations"]["slope"].split())
            ),  # change in log-sampling rate through time (log-linear)
            intercept=list(
                map(float, config["simulations"]["intercept"].split())
            ),  # initial sampling rate
            sd_through_time=list(
                map(float, config["simulations"]["sd_through_time"].split())
            ),
            # st dev in log-sampling rate through time
            sd_through_time_skyline=config.getfloat(
                "simulations", "sd_through_time_skyline"
            ),
            mean_rate_skyline=list(
                map(float, config["simulations"]["mean_skyline_sampling"].split())
            ),
            mean_n_epochs_skyline=config.getfloat(
                "simulations", "mean_n_epochs_skyline"
            ),
            fraction_skyline_sampling=config.getfloat(
                "simulations", "fraction_skyline_sampling"
            ),
            bin_sampling=freq_bin_sampling,
            bin_std_rates=bin_std_rates,
            bin_mean_rates=bin_mean_rates,
            maximum_localities_per_bin=config.getint(
                "simulations", "maximum_localities_per_bin"
            ),
            ...
            target_n_occs_range=target_n_occs_range,
            seed=rseed,
        )  # if > 0 fixes the random seed to make simulations reproducible
...
def run_sim_from_config(config, customize_bd_obj: custom_bd = None):
    ...
    bd_sim, fossil_sim = create_sim_obj_from_config(
        config, rseed=config.getint("simulations", "training_seed")
    )
    ...
    if "region_constraints" in config.sections():
        ...
        if len(area_start):
            ...
            for i in range(len(area_start)):
                if config["region_constraints"][area_start[i]] != "NA":
                    val = list(
                        map(float, config["region_constraints"][area_start[i]].split())
                    )
                    min_val = np.min(val)
                    max_val = np.max(val)
        ...
        if len(area_end):
            if area_tbl is None:
                ...
                area_tbl[:, 1:] = (
                    -1
                )  # set all values to -1 (all areas exist throughout)
                area_tbl_max = area_tbl + 0

            for i in range(len(area_end)):
                if config["region_constraints"][area_end[i]] != "NA":
                    val = list(
                        map(float, config["region_constraints"][area_end[i]].split())
                    )
                    min_val = np.min(val)
                    max_val = np.max(val)
    ...
    if area_tbl is not None:
        area_constraint = {"min_age": area_tbl, "max_age": area_tbl_max}
    else:
    ...
    try:
        os.mkdir(
            os.path.join(config["general"]["wd"], config["simulations"]["sims_folder"])
        )
    except FileExistsError:
        pass
    f, l, d = save_simulations(
        res,
        os.path.join(config["general"]["wd"], config["simulations"]["sims_folder"]),
        config["simulations"]["sim_name"] + "_" + now + "_training",
        return_file_names=True,
    )
    return f, l, d


def run_test_sim_from_config(config, customize_bd_obj: custom_bd = None):
    # simulate test data
    bd_sim, fossil_sim = create_sim_obj_from_config(
        config, rseed=config.getint("simulations", "test_seed")
    )
    ...
    test_set = sim_settings_obj(
        ...
        keys=[
            ...
            "eta",
            "p_gap",
            "area_size_concentration_prm",
            "link_area_size_carrying_capacity",
            "slope_log_sampling",
            "intercept_initial_sampling",
            "sd_through_time",
            "additional_info",
        ],
        include_present_diversity=include_present_diversity,
    ...
    f, l, d = save_simulations(
        res,
        os.path.join(config["general"]["wd"], config["simulations"]["sims_folder"]),
        config["simulations"]["sim_name"] + "_" + now + "_test",
        return_file_names=True,
    )
    return f, l, d
...
def get_model_settings_from_config(config, total_diversity=False, model_number=1):
    ...
    lstm_nodes = np.array(
        list(map(int, config["model_training"]["lstm_model_%s" % model_number].split()))
    )
    arrays = [[lstm_nodes[0]]]
    indx = 0
    ...
    dense_nodes = np.array(
        list(
            map(int, config["model_training"]["dense_model_%s" % model_number].split())
        )
    )
...
def run_model_training_from_config(
    config,
    feature_file=None,
    label_file=None,
    convert_to_tf=True,
    model_tag=None,
    return_model_dir=False,
    calibrate_output=False,
    total_diversity=None,
    label_rescaler=None,
    verbose=1,
    model_number=1,
):
    ...
    model_settings = get_model_settings_from_config(
        config, total_diversity=total_diversity, model_number=model_number
    )
    sims_path = os.path.join(
        config["general"]["wd"], config["model_training"]["sims_folder"]
    )
    if feature_file is None:
        ...
        if config["model_training"]["f"] == "NULL":
            sys.exit(
                "No feature or label files specified, provide to run_model_training or in the config (see R)"
            )
    model_wd = os.path.join(
        config["general"]["wd"], config["model_training"]["model_folder"]
    ).replace("\\", "/")
    ...
    try:
        ...
        Xt = Xt[
            :,
            :,
            0 : len(get_features_names(n_areas=config.getint("general", "n_regions"))),
        ]
    ...
    if calibrate_output:
        ...
        present_div_vec = np.einsum(
            "i, ib -> ib", Xt_r[:, 0, -1], np.ones((Xt_r.shape[0], Xt_r.shape[1]))
        )
        dict_inputs = {
    ...
    plot_training_history(
        history, criterion="val_loss", wd=model_dir, show=False, filename=out_name
    )
...
def predict_from_config(
    ...
    model_tag="",
    model_dir_id="rnn_model",
    calibrated=False,
    return_transformed_diversity=False,
    model_dir=None,
    label_rescaler=None,
):
    dd_input = os.path.join(
        config["general"]["wd"], config["empirical_predictions"]["empirical_input_file"]
    )
    if model_dir is not None:
        ...
        loaded_models = load_models(
            model_wd=os.path.join(
                config["general"]["wd"], config["empirical_predictions"]["model_folder"]
            ),
    ...
    for model_i in range(len(loaded_models)):
        ...
        try:
            pred_div = predict(
                ...
                model,
                feature_rescaler,
                n_predictions=config.getint("empirical_predictions", "n_predictions"),
                dropout=False,
...
def predict_testset_from_config(
    config,
    test_feature_file,
    test_label_file,
    model_tag="",
    ...
    for model_i in range(len(loaded_models)):
        ...
        pred_div = predict(
            features, model, feature_rescaler, dropout=False, calibrated=calibrated
        )
...
def config_autotune(config_init, target_n_occs_range=10):
    ...
    feat_emp = parse_dd_input(dd_input, present_diversity=pres_div)

    feat_names_df = get_features_names(
        n_areas=n_areas, include_present_div=True, as_dataframe=True
    )
    feat_names = get_features_names(
        n_areas=n_areas, include_present_div=True, as_dataframe=False
    )
    ...
    if config["simulations"]["disp_rate_mean"] == "NA":
        config["simulations"]["disp_rate_mean"] = "%s %s" % (
            0.2 * (1 - prop_endemics),
            5 * (1 - prop_endemics),
        )
    ...
    indx = np.array(
        [i for i in range(len(feat_names)) if "n_locs_area_" in feat_names[i]]
    )
    # print(feat_names)
    n_localities_area = feat_emp[0][:, indx]  # shape (t, a)

    print("n_localities_area: ", n_localities_area.shape)
    # print(nbvcx)

    indx = np.array(
        [i for i in range(len(feat_names)) if "n_species_area_" in feat_names[i]]
    )
    n_species_area = feat_emp[0][:, indx]

    indx = np.array(
        [i for i in range(len(feat_names)) if "n_occs_area_" in feat_names[i]]
    )
    ...
    for i in range(n_areas):
        ...
        slope, intercept, _, __, ___ = scipy.stats.linregress(
            time_bins[1:], np.log(n_localities_area[:, i] + 1)
        )
        slopes.append(slope)
    ...
    if config["simulations"]["intercept"] == "NA":
        config["simulations"]["intercept"] = "%s %s" % (
            np.min(np.exp(intercepts)) / 2,
            np.max(np.exp(intercepts)) * 2,
        )
    ...
    if config["simulations"]["maximum_localities_per_bin"] == "NA":
        config["simulations"]["maximum_localities_per_bin"] = "%s" % int(
            np.max(n_localities_area)
        )

    # re-set mean skyline sampling
    if config["simulations"]["mean_skyline_sampling"] == "NA":
        config["simulations"]["mean_skyline_sampling"] = "%s %s" % (
            np.log(np.mean(n_localities_area) / 2),
            np.log(np.mean(n_localities_area) * 2),
        )
    if config["simulations"]["sd_through_time_skyline"] == "NA":
        config["simulations"]["sd_through_time_skyline"] = "%s" % np.std(
            np.log(n_localities_area + 1)
        )
    ...
    if config["simulations"]["dd_K"] == "NA":
        config["simulations"]["dd_K"] = "%s %s" % (
            int(np.mean(range_through_div[range_through_div > 0]) / 2),
            np.maximum(min_div, int(np.max(range_through_div) * 5)),
        )

    # re-set per-species sampling rate
    m = (n_species - n_singletons)[range_through_div > 0] / range_through_div[
        range_through_div > 0
    ]
    ...
    if config["simulations"]["p_gap"] == "NA":
        config["simulations"]["p_gap"] = "%s %s" % (
            0,
            np.sum(n_occs_area == 0) / np.size(n_occs_area),
        )
    ...
    if pres_div is not None:
        if pres_div > 0 and config["simulations"]["extant_sp"] == "NA":
            config["simulations"]["extant_sp"] = "%s %s" % (
                int(pres_div / 2),
                int(pres_div * 10),
            )
    # if pres_div == 0:
    ...
    if pres_div is not None:
        if pres_div == 0 and config["simulations"]["survive_age_condition"] == "NA":
            ...
            config["simulations"]["survive_age_condition"] = (
                "%s" % min_age_with_occurrences
            )

    if config["simulations"]["total_sp"] == "NA":
        config["simulations"]["total_sp"] = "%s %s" % (
            int(np.max(n_species) * 2),
            int(np.sum(n_species) * 20),
        )
    ...
    try:
        if config["simulations"]["use_empirical_sampling"] == "TRUE":
            n_localities_area_file = "n_localities_area.npy"
            np.save(
                os.path.join(config["general"]["wd"], n_localities_area_file),
                np.log(0.001 + n_localities_area).T,
            )

            x_pd = pd.DataFrame(n_localities_area)
            x_pd.to_csv(
                os.path.join(
                    config["general"]["wd"],
                    n_localities_area_file.replace(".npy", ".csv"),
                )
            )

            print(
                "Empirical locality rates saved in :",
                os.path.join(config["general"]["wd"], n_localities_area_file),
            )
            config["simulations"]["locality_data_file"] = n_localities_area_file
    except:
        pass
    ...
    if config["simulations"]["bin_sampling"] == "NA":
        config["simulations"][
            "bin_sampling"
        ] = "0.67"  # 50% of simulations overall with empirical loc rates

    if config["simulations"]["s_species"] == "NA":
        config["simulations"]["s_species"] = "1 %s" % np.maximum(
            min_div, int(np.max(range_through_div) * 5)
        )
